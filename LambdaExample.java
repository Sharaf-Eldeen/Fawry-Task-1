...
public class LambdaExample {
    ...
    public static void main(final String[] args) {
        ...
        // TODO With functional interfaces declared
        Comparator<User> sortByNameAsc = new Comparator<User>() {
            @Override
            public int compare(User u1, User u2) {
                return u1.name.compareTo(u2.name);
            }
        };
        repository.select(activeUserPredicate, sortByNameAsc);
        ...
        Comparator<User> sortByNameDesc = new Comparator<User>() {
            @Override
            public int compare(User u1, User u2) {
                return u1.name.compareTo(u2.name);
            }
        };
        repository.select(activeAndAgeGreaterThan8, sortByNameDesc);

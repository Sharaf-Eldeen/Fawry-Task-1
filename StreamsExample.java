...
public class StreamsExample {
    ...
    public static void main(final String[] args) {
        ...
        // TODO With functional interfaces declared
        OptionalDouble averagePrice = authors
            .stream()
            .flatMap(author -> author.books.stream())
            .mapToInt(new java.util.function.ToIntFunction<Book>() {
                @Override
            ...
            .average();
        System.out.println("Average Price: $" + (averagePrice.isPresent() ? averagePrice.getAsDouble() : 0));

        banner("Average price for all books in the library - lambda");
        // TODO With functional interfaces used directly
        averagePrice = authors
            .stream()
            .flatMap(author -> author.books.stream())
        ...
        // TODO With functional interfaces declared
        Predicate<Author> activeAuthorWithBooksPredicate = new Predicate<Author>() {
            @Override
            public boolean test(Author author) {
        ...
        authors
            .stream()
            .filter(activeAuthorWithBooksPredicate)
            .forEach(System.out::println);
